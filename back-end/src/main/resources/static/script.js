...
function resetForm() {
  const inputs = document.querySelectorAll(
    "#myform input:not([type='submit']), #myform select",
  );
...
function createMoreOrderField() {
  Ordercount++;
  let OrderList = document.getElementById("OrderListBody");
...
function calculateTotal() {
  ...
  for (let i = 1; i <= Ordercount; i++) {
    let totalField = document.getElementById(`orderTotal${i}`);
    if (totalField) {
      if (totalField.value) total = total + parseInt(totalField.value);
    }
  }
...
function fillProductId(count) {
  ...
  let suggestion = document.getElementById(`OrderSuggestion${count}`);
  let productId = document.getElementById(`orderProductName${count}`).value;
  if (productId.trim() !== "") {
...
function showOrderSuggestion(products, count) {
  let suggestion = document.getElementById(`OrderSuggestion${count}`);
  let productName = document.getElementById(`orderProductName${count}`);
  let productId = document.getElementById(`orderProductId${count}`);
  let productPrice = document.getElementById(`orderProductPrice${count}`);
  let quantity = document.getElementById(`orderProductQuantity${count}`);
  suggestion.innerHTML = "";
...
function calculatePrice(count) {
  let price = document.getElementById(`orderProductPrice${count}`).value;
  let quantity = document.getElementById(`orderProductQuantity${count}`).value;
  let total = document.getElementById(`orderTotal${count}`);
...
function viewProduct(event) {
  ...
  let storeId = inputstoreId.value;
...
function filter() {
  let productName = document.getElementById("searchBar").value;
...
async function createData(products, storeId) {
  ...
  products.forEach((product) => {
    ...
    const stockLevelValue =
      inventoryList.length > 0 ? inventoryList[0].stockLevel : 0;
...
function fillProductName() {
  ...
  if (productName.trim() != "") {
    ...
    let suggestion = document.getElementById("aStoreSuggestion");
    suggestion.innerHTML = "";
  }
}
...
function addProductToInventory(event) {
  ...
  let storeId = document.getElementById("astoreId").value;
  let stockLevel = document.getElementById("astockLevel").value;
...
function showModal(id, name, action) {
  ...
  let text = document.getElementById("deleteProuctName");
  text.innerHTML = name;
...
function validateQuantity(OrderNo) {
  let quantity = document.getElementById(
    `orderProductQuantity${OrderNo}`,
  ).value;
  let storeId = document.getElementById("orderStoreId").value;
  let productId = document.getElementById(`orderProductId${OrderNo}`).value;
...
async function placeOrder(event) {
  ...
  for (let i = 1; i <= Ordercount; i++) {
    ...
    let orderProductName = document.getElementById(
      `orderProductName${i}`,
    ).value;
    let orderProductId = document.getElementById(`orderProductId${i}`).value;
    let orderProductPrice = document.getElementById(
      `orderProductPrice${i}`,
    ).value;
    let orderProductQuantity = document.getElementById(
      `orderProductQuantity${i}`,
    ).value;
